...
function detectApps() {
  ...
  const text_tests = {
    ...
    GetSatisfaction: /asset_host\s*\+\s*"javascripts\/feedback.*\.js/gim,
  ...
  // convert to array
  const jsonString = JSON.stringify(_apps)
  // send back to background page
  const meta = document.getElementById('whatsthetech_meta')
  meta.content = jsonString

  //Notify Background Page
  const done = new Event('whatsthetechEvent', { bubbles: true, cancelable: true })
